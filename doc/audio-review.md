...
> ⚠️ **위 표의 피아노·기타 "LRU 캐시", "잘 됨" 평가는 더 이상 코드와 맞지 않습니다. (2026-09-01 확인)**
> 이 리뷰 **다음 날** `b48fec8`에서 LRU(`CACHE_LIMIT = 15`)와 폴리포니 임시 플레이어가 삭제되고
> **52개(피아노) / 28개(기타) 전체 프리로드**로 바뀌었습니다. 그것이 앱 전체 무음의 원인이었습니다.
> 자세한 내용은 문서 하단 "정정 기록"과 `doc/audio-무음-원인과-방향.md`를 보세요.
...
## 2026-09-01 — 요약 표의 피아노·기타 평가는 유효기간이 지났다

**무엇이 틀렸나**

요약 표에서 피아노·기타를 "expo-audio 직접 + LRU 캐시 + 폴리포니 / 잘 됨"으로 적었습니다.
**작성 시점(2026-08-14)에는 맞는 서술이었습니다.** 문제는 그 코드가 **다음 날 사라졌는데
이 문서가 갱신되지 않은 것**입니다.

```
b48fec8 (2026-08-15 23:32, "doc_78_주석지우기전guitar")
  - const CACHE_LIMIT = 15;
  - const recentlyUsedNotes = ...        ← LRU 삭제
  - tempPlayers (폴리포니 임시 플레이어)  ← 삭제
  + useFocusEffect(... 52개 전부 createAudioPlayer)   ← 전체 프리로드
```

**실제로 무슨 일이 일어났나**

안드로이드는 **앱(UID)당 동시 AudioTrack을 약 40개로 제한**합니다.
`createAudioPlayer`는 재생 여부와 무관하게 **만드는 즉시** 트랙 하나를 잡습니다.
피아노 52 + 기타 28 + 배경 상주 26(동물 12 · 단어 8 · 드럼 5 · 냉장고 1) = **최악 109개**.
한도를 넘긴 플레이어는 만들어지자마자 죽고, 그 뒤 `play()`는 **예외도 로그도 없이 무음**입니다.

실기기 logcat(SC-01M / Android 12)으로 확인된 사실입니다. 증거·타임라인·수정 방향은
`doc/audio-무음-원인과-방향.md`에 있습니다.

**왜 이 문서가 해로웠나**

2026-09-01 세션에서 낙하모드 무음을 조사할 때, 이 문서를 먼저 읽었다면
"피아노는 잘 됨"이라는 표를 보고 **피아노를 용의선상에서 뺐을 것**입니다.
— 실제로는 피아노가 원인이었습니다.

> **리뷰 문서는 스냅샷이다.** 대상 코드가 바뀌면 평가도 같이 죽는다.
> 특히 "잘 됨" 판정은 **무엇을 근거로 잘 됐다고 했는지**(여기서는 LRU 상한)를 함께 적어야,
> 그 근거가 사라졌을 때 평가가 무효라는 걸 알 수 있다.

**고친 것**

`3-branch`에서 LRU(`CACHE_LIMIT = 12`)와 재생 에러 시 죽은 플레이어 축출을 복원했고
(`0b6c216` · `370359c`), 2026-09-01에 이 브랜치로 가져왔습니다.
